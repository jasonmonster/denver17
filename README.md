...
**Page-lookup bug (get_page_by_path, then get_posts):** Two separate bugs stacked on top of each other in the same slug-lookup helper across `bin/setup.php`, `bin/rebuild-menus.php`, and `bin/populate-inner-pages.php`.

First: `get_page_by_path()` reconstructs a page's full ancestor path and compares it against the string you pass in. Given a bare slug like `'facilities'`, it only matches a page with no parent — a nested page's real path is `visit/facilities`, which doesn't equal `facilities`, so the lookup silently returns nothing for every nested page.

Swapping that for `get_posts( [ 'name' => $slug, 'post_status' => 'any', ... ] )` looked like the fix, but surfaced a second bug: `'post_status' => 'any'` in `WP_Query` doesn't mean literally any status. For non-public statuses (draft, pending, private), it only includes them if the querying user has permission to see them — otherwise it silently restricts to that user's own posts. WP-CLI runs with no logged-in user by default (ID 0), so every draft-status lookup returned nothing, while the two pages that happened to be published kept working. Looked identical to the first bug from the CLI output (duplicate pages on rerun) but had nothing to do with hierarchy.

Both are fixed the same way now: a direct `$wpdb->get_var()` query against `wp_posts` for `post_name` + `post_type = 'page'` + `post_status != 'trash'`. No hierarchy comparison, no capability check. If a future script needs to find a page by slug, use that pattern — not `get_page_by_path()`, and not `get_posts()` with `'post_status' => 'any'` unless it's guaranteed to run as a user with edit capabilities.

**Stale asset cache (inc/enqueue.php):** `main.css` and `main.js` were enqueued with `wp_get_theme()->get( 'Version' )` as the cache-busting version string — the theme's static version number, which doesn't change between deploys. Once a browser (or any CDN/proxy in front of the site) cached `main.css?ver=0.1.0`, it kept serving that exact copy indefinitely, since the URL never changed even after the file's contents did. Several CSS fixes appeared not to be working purely because of this — the server had the correct file the whole time, confirmed by fetching it directly with a cache-busting query param. Fixed by switching both assets to `filemtime()` of their own file as the version, so the query string changes automatically on every deploy. If a similar caching mystery shows up again, fetch the asset directly with `?cachebust=<timestamp>` before assuming the deploy didn't land.

**Block schema defaults override "empty" checks (feature-split):** `feature-split`'s `linkText` attribute had `"default": "Learn more"` in `block.json`. WordPress merges a block's registered schema defaults into `$attributes` before calling its render callback — regardless of whether the saved block comment includes that key. So checking `! empty( $attributes['linkText'] )` in PHP to decide whether to show a link never worked, `linkText` was never actually empty at that point, it was always at least `"Learn more"` from the schema. Looked exactly like a caching/deploy problem (the fix was "correct" and just never seemed to take effect) but had nothing to do with deployment. Fixed by changing the schema default to `""` so an unset link is actually distinguishable from a real one. If a future block needs an optional field like this, the default in `block.json` has to be the true "nothing set" value, not a placeholder.
